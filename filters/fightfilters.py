...
class NonSpellFilter:
    ...
    def parse(self, log_line):
        for expression in self.regexes:
            result = re.search(expression, log_line.get('text'))
            if result:
                filter_type = 'hit' if result.group('amount').isnumeric() else 'miss'
                timestamp = log_line.get('timestamp')
                parsed_data = {
                    self.columns[0]: timestamp,
    ...
    def parse(self, log_line):
        for expression in self.regexes:
            ...
            if result:
                ...
                parsed_data = {
                    self.columns[0]: timestamp,
                    self.columns[1]: result.group(1).replace('YOUR', 'You'),
                    self.columns[2]: result.group(2).replace('YOU', 'You'),
                    self.columns[3]: result.group(3),
                    self.columns[4]: result.group(5),
